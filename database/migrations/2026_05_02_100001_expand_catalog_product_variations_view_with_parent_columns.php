...
/**
 * Expands catalog.product_variations_view with parent product context for the
 * standalone GET /api/products/variations endpoint.
 *
 * Adds SQL-computed variation_title and three filter columns (parent_is_active,
 * parent_has_free_delivery, parent_main_category_ids) needed for WHERE clauses.
 * Remaining parent context (images, custom_fields, url, sku, vat) is accessed
 * via Eloquent eager-loading of the parent ProductModel.
 *
 * PostgreSQL views can't be ALTERed — must DROP + full CREATE.
 *
 * Down restores the 2026_05_01_100005 definition (popularity migration).
 */
