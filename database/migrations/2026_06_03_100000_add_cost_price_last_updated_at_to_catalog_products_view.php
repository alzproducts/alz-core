...
/**
 * Adds `cost_price_last_updated_at` to catalog.products_view.
 *
 * Source: catalog.cost_price_changes.changed_at — the append-only audit trail of supplier
 *         cost-price deltas written by UpdateCostPriceBySupplierUseCase (#876). Every row is a
 *         real change, so (unlike price_periods) there is no SCD2 "current row" filter — take MAX.
 * Scope:  parent SKU + every variation SKU belonging to the product. A plain JOIN on p.sku misses
 *         variant-only products (no parent SKU), so the LATERAL collects all SKUs via UNION ALL
 *         before joining to cost_price_changes — mirrors the existing price_history lateral.
 *
 * NULL when no SKU has a cost_price_changes row yet (cost price never changed through that flow) —
 * consistent with price_last_updated_at returning NULL when no price_periods row exists.
 *
 ...
 *       (from 2026_05_19_100000_add_price_last_updated_at_to_catalog_products_view) is restored verbatim.
 *
 * Precedent for additive view migrations: 2026_05_19_100000_add_price_last_updated_at_to_catalog_products_view.php.
 */
return new class extends Migration {
    ...
    public function down(): void
    {
        ...
        // Restored verbatim from 2026_05_19_100000_add_price_last_updated_at_to_catalog_products_view::up()
        // (the products_view CREATE block, with price_last_updated_at but without cost_price_last_updated_at).
