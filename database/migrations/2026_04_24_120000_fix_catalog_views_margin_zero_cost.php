...
/**
 * Tightens the margin guard on both catalog views so that a zero supplier
 * purchase_price does not produce profit_margin = 100.
 *
 * Previous guard `s.purchase_price IS NOT NULL` passed for rows where the
 * supplier cost is literally 0.00, causing (price - 0) / price * 100 = 100.
 * New guard `s.purchase_price > 0` excludes zero and NULL together.
 *
 * Both catalog.products_view and catalog.product_variations_view share the
 * same bug and are recreated here. No JOIN / CTE / column-list changes —
 * only the CASE guard on the profit_margin column.
 *
 * Up bodies are copied verbatim from the most recent live definitions:
 *   - catalog.products_view          — 2026_04_21_041810_add_popularity_to_catalog_products_view::up()
 *   - catalog.product_variations_view — 2026_04_18_024602_add_available_physical_stock_to_catalog_product_views::up()
 *
 * Down restores those same definitions verbatim (with the IS NOT NULL guard)
 * so rollback returns the schema to the pre-migration state exactly.
 *
...
return new class extends Migration {
    ...
    public function down(): void
    {
        ...
        // 2026_04_21_041810_add_popularity_to_catalog_products_view::up() (verbatim, with IS NOT NULL guard).
        DB::statement(<<<'SQL'
            CREATE VIEW catalog.products_view AS
        ...
        // 2026_04_18_024602_add_available_physical_stock_to_catalog_product_views::up() (verbatim, with IS NOT NULL guard).
        DB::statement(<<<'SQL'
            CREATE VIEW catalog.product_variations_view AS
