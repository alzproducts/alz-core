...
C4Context
    ...
    System_Ext(clickup, "ClickUp", "Task management — encrypted API keys")
    ...
    Rel(alzCore, clickup, "Reads/writes tasks", "REST API + API key")
    ...
    Rel(alzCore, cloudflare, "Stores/retrieves objects", "S3-compatible API")
```
...
C4Container
    ...
    Container_Boundary(railway, "Railway Platform") {
        ...
        Container(scheduler, "Scheduler Service", "schedule:work", "Cron-triggered tasks across 10 schedule providers")
...
Five queue tiers route jobs by urgency: `high` (time-sensitive, user-facing — webhooks, notifications), `default` (normal — order sync, daily jobs), `bulk` (high-volume small jobs), `low` (bulk/background work — full customer sync, migrations), and `background` (ultra-long-running — historical backfills, full PO syncs). Four Horizon supervisors map onto these tiers. See `config/horizon.php`.
...
C4Component
    ...
    Container_Boundary(application, "Application Layer") {
        ...
        Component(appcommands, "Commands", "Write parameter objects", "ProductFilterChangeCommand, LeadConversionCommand, GenerateVariantSkusCommand — encapsulate write operations")
        Component(contracts, "Contracts", "Cross-layer interfaces", "Client, repository, and dispatcher interfaces — Infrastructure implements; Application depends on the abstraction")
    }
    ...
    Container_Boundary(domain, "Domain Layer") {
        ...
        Component(events, "Events", "Domain events", "ProductPricingUpdatedEvent, ContactFormProcessedEvent, VariantSkusGeneratedEvent")
        Component(validators, "Validators", "Business rule checks", "MoneyEqualsValidator, PriceChangedValidator, SkuBelongsToProductValidator — return Result objects")
        Component(exceptions, "Exceptions", "Business rules", "Transient/Permanent API failure hierarchy, domain violations")
    }

    Container_Boundary(infrastructure, "Infrastructure Layer") {
        Component(clients, "API Clients", "External communication", "ShopWired, Linnworks, Google Ads, Bing Ads, Mixpanel, HelpScout, Reviews.io, ClickUp, Cloudflare R2")
        ...
        Component(mappers, "Mappers", "DTO ↔ entity transformation", "Per-integration mappers under Catalog/{Brand,Order,Product,Category}/Mappers, HelpScout/Mappers, Linnworks/Mappers, etc.")
        Component(models, "Persistence Models", "Eloquent ORM models", "Persistence/Models/{Auth, Config, System} — AuthAllowedDomainModel, ProfileModel, DashboardConfigModel, SystemCacheModel, etc.")
    }
...
flowchart LR
    ...
    CTRL --> SVC["Per-entity HandleWebhookService"]
...
Each ShopWired entity type (order, product, customer, brand, category) has its own webhook controller and `Handle{Entity}WebhookService` that routes by event topic to the appropriate use case (sync, delete, status change, refund). The webhook saves the partial payload synchronously, dispatches a re-fetch job to pull the full entity from the API, and returns `200 OK` immediately.
