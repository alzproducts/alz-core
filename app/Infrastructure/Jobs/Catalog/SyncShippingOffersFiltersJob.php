...
/**
 * Hourly orchestrator: query products with changed Shipping Offers filters and dispatch per-entity updates.
 *
 * Does NOT call the ShopWired API directly — only queries the database view
 * and dispatches UpdateProductFilterJob per product.
 */
final class SyncShippingOffersFiltersJob extends AbstractJob implements ShouldBeUnique
{
    public int $tries = 4;
    ...
    {
        return 'sync-shipping-offers-filters';
    }
     ...
     */
    public function handle(SyncShippingOffersFiltersUseCase $useCase): void
    {
        $useCase->execute();
    }
}
