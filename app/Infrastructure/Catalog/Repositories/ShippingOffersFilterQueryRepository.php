...
/**
 * Queries the catalog.products_with_changed_shipping_offers_filters Postgres view.
 *
 ...
 * (not text[]) because Shipping Offers filter values contain whitespace.
 */
final class ShippingOffersFilterQueryRepository implements ShippingOffersFilterQueryRepositoryInterface
{
    ...
    public function getProductsWithChangedShippingOffersFilters(): array
    {
        ...
        $rows = $this->eloquentGateway->query(static fn(): array => self::MODEL_CLASS::query()
            ->getConnection()
            ->select('SELECT product_id, desired_filter_values FROM catalog.products_with_changed_shipping_offers_filters'));

        return self::mapRowsToCommands($rows, FilterGroupOptionNo::ShippingOffers->value);
    }
    ...
    private static function mapRowsToCommands(array $rows, int $optionNo): array
    {
        return \array_map(
            static fn(object $row): ProductFilterChangeCommand => new ProductFilterChangeCommand(
                ...
                optionNo: $optionNo,
                desiredFilterValues: ShippingOffersFilterValue::fromJsonArray($row->desired_filter_values),
            ),
            $rows,
        );
    }
