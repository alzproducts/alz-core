 ...
 * Uses 3-tier frequency: daily (full sync), hourly (quick sync), micro (near real-time).
 *
 * Daily syncs run during 04:00-07:00 UK time window. Micro/hourly syncs are
...
final class ShopwiredScheduleServiceProvider extends ServiceProvider
{
    ...
    {
        $skipDuringDailySync = $this->createDailySyncWindowCheck();

        $this->registerOrderSchedules($skipDuringDailySync);
        $this->registerCustomerSchedules($skipDuringDailySync);
    }

    /**
     * Check if currently in ShopWired daily sync window (04:00-07:00 UK).
     * Used to skip micro/hourly syncs during full sync to avoid rate limit contention.
     */
    private function createDailySyncWindowCheck(): Closure
    {
     ...
     * @throws RuntimeException
     */
    private function registerOrderSchedules(Closure $skipDuringDailySync): void
    {
     ...
     * @throws RuntimeException
     */
    private function registerCustomerSchedules(Closure $skipDuringDailySync): void
    {
